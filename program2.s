ldi r0, #4  		// Func2_start: Prepare to load 30 = 0011110
ls r0, #4
ldi r1, #7			// Prepare to store into data mem[225] = 111000001
ls r1, #5
addi r1, #1
str r1, r0			// store 30 for later
ldi r0, #5			// load 0 = 0000000
ls r0, #3
addi r0, #7
ls r0, #1
addi r1, #1     // Prepare to store into data mem[226] = 11100010 inc from 225
str r1, r0			// store 0 for later
...
addi r2, #2
str r2, r1
ldi r0, #7			// check if we're done by loading final output address: 30 = 0011110
ls r0, #3
addi r0, #6
ls r0, #1
